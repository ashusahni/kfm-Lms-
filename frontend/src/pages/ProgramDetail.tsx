...
import { formatProgramPrice } from "@/lib/formatPrice";
...
const ProgramDetail = () => {
  ...
  const addToCartMutation = useMutation({
    mutationFn: () => cartService.add(id!, undefined, selectedBatchId ?? undefined),
    onSuccess: () => {
  ...
  const contentList = Array.isArray(content)
    ? content
    : (content as { content?: unknown[] })?.content ?? (content as { data?: unknown[] })?.data ?? [];
          ...
          <section className="mt-8">
            <div className="rounded-2xl overflow-hidden bg-muted shadow-card border border-border">
              <div className="relative aspect-video">
                ...
                {detail.challenge_type === "7_days" ? (
                  <span className="absolute top-4 left-4 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-sm font-semibold">
                    7 Day Challenge
                  </span>
                ) : detail.challenge_type === "30_days" ? (
                  <span className="absolute top-4 left-4 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-sm font-semibold">
                    30 Day Challenge
                  </span>
                ) : null}
            ...
            <div className="mt-6 flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
              <div className="flex-1 min-w-0">
                ...
                <p className="mt-2 text-gray-600">
                  {detail.teacher?.headline ?? "Learn with expert dieticians."}
                </p>
            ...
            <div className="lg:col-span-1">
              <div className="lg:sticky lg:top-24 space-y-6">
                  ...
                  <div className="p-6">
                    <div className="flex items-baseline gap-2 flex-wrap">
                      <span className="text-3xl font-bold text-gray-900">
                        {formatProgramPrice({
                          priceString: detail.best_ticket_string ?? detail.price_string ?? null,
                          price: detail.price,
                          currencySign: appConfig?.currency?.sign,
                          currencyPosition: appConfig?.currency_position,
                          decimals: appConfig?.currency_decimal ?? 0,
                        })}
                      </span>
                      {detail.discount_percent && detail.discount_percent > 0 && (
                        <span className="text-gray-500 line-through text-lg">
                          {formatProgramPrice({
                            priceString: detail.price_string,
                            price: detail.price,
                            currencySign: appConfig?.currency?.sign,
                            currencyPosition: appConfig?.currency_position,
                            decimals: appConfig?.currency_decimal ?? 0,
                    ...
                    <div className="mt-6 space-y-3">
                      {detail.auth_has_bought ? (
                        <Link to={`/panel/learn/${detail.id}`} className="block">
                        ...
                        <>
                          {(() => {
                            const openBatches = (detail.batches ?? []).filter((b) => b.is_open);
                            const hasOpenBatches = openBatches.length > 0;
                            const batchRequired = hasOpenBatches;
                            const batchSelected = selectedBatchId != null && selectedBatchId > 0;
                            const canSubmit = !batchRequired || batchSelected;
                            return (
                              <>
                                {hasOpenBatches && (
                                  <div className="space-y-2">
                                    <label className="text-sm font-medium text-gray-700">Batch <span className="text-destructive">*</span></label>
                                    <select
                                      className="w-full h-11 rounded-xl border border-gray-200 bg-white px-4 text-gray-900"
                                      value={selectedBatchId ?? ""}
                                      onChange={(e) => setSelectedBatchId(e.target.value ? Number(e.target.value) : null)}
                                      aria-invalid={batchRequired && !batchSelected}
                                    >
                                      <option value="">— Select batch —</option>
                                      {openBatches.map((batch) => (
                                        <option key={batch.id} value={batch.id}>
                                          {batch.start_date_formatted
                                            ? `${batch.name} (Starts ${batch.start_date_formatted})`
                                            : batch.name}
                                          {batch.capacity != null ? ` (${batch.enrolled_count ?? 0}/${batch.capacity})` : ""}
                                        </option>
                                      ))}
                                    </select>
                                    {batchRequired && !batchSelected && (
                                      <p className="text-sm text-destructive">Please select a batch to continue.</p>
                                    )}
                                  </div>
                                )}
                                <Button
                                  className="w-full bg-primary hover:bg-primary/90 text-white h-12 font-semibold rounded-xl"
                                  disabled={addToCartMutation.isPending || !canSubmit}
                ...
                {detail.teacher && (
                  <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-100">
                      <h3 className="font-semibold text-gray-900">Dietician</h3>
                    </div>
                    <div className="p-6 flex gap-4">
                      <img
                        src={detail.teacher.avatar ?? ""}
                        alt={detail.teacher.full_name ?? "Dietician"}
