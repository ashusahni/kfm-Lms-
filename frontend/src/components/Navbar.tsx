...
const Navbar = () => {
  ...
  const { isAuthenticated, isInstructor, logout, fullName } = useAuth();
  const dashboardPath = isInstructor ? "/dietician" : "/panel";
              ...
              <>
                ...
                <Link to={dashboardPath}>
                  ...
                  >
                    ...
                    {isInstructor ? "Dietician" : "Dashboard"}
                    {fullName && (
                      <span className="hidden sm:inline text-foreground/80">({fullName})</span>
                    )}
                  </Button>
                </Link>
                <Button
                  ...
                  <>
                    ...
                    <Link to={dashboardPath} onClick={() => setOpen(false)}>
                      <div className="flex items-center gap-3 rounded-xl px-4 py-3.5 text-[15px] font-medium text-muted-foreground hover:bg-muted hover:text-foreground">
                        ...
                        {isInstructor ? "Dietician" : "Dashboard"}
                        {fullName && <span className="text-foreground/80"> ({fullName})</span>}
                      </div>
                    </Link>
                    <button
                      type="button"
