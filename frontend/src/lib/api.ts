...
const AUTH_USER_NAME_KEY = "rocket_lms_user_name";
...
export function setAuthUserName(name: string | null): void {
  if (name === null) localStorage.removeItem(AUTH_USER_NAME_KEY);
  else localStorage.setItem(AUTH_USER_NAME_KEY, name);
}
 ...
 * Pass skipContentType: true for FormData uploads.
 */
function buildHeaders(init?: RequestInit, token?: string | null, skipContentType?: boolean): HeadersInit {
...
export async function requestRaw<T = unknown>(
  ...
  const res = await fetch(url, {
    ...init,
    headers: buildHeaders(init, undefined, (init?.body as FormData) instanceof FormData),
  });
...
export const apiRaw = {
  ...
  postFormData: <T = unknown>(path: string, formData: FormData, options?: RequestOptions) =>
    requestRaw<ApiResponse2<T>>(path, {
      ...options,
      method: "POST",
      body: formData,
