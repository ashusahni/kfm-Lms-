...
| **CollisionServiceProvider not found (500 on deploy)** | Cached `bootstrap/cache/packages.php` listed dev-only packages (e.g. `nunomaduro/collision`). Production uses `composer install --no-dev`, so those classes aren't installed. The Dockerfile now removes `packages.php` and `services.php` before start so Laravel re-discovers only installed packages. **Redeploy** after pulling the fix. If the error persists, use Render → **Manual Deploy** → **Clear build cache & deploy** so Docker doesn’t reuse old layers and the fix is applied. |
