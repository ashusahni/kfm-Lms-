...
Route::group([], function () {
    ...
    Route::get('/recommended-meals', ['uses' => 'RecommendedMealsController@index']);
    Route::get('/recommended-meals/{assignmentId}', ['uses' => 'RecommendedMealsController@show']);
    ...
    Route::get('/my-batches', ['uses' => 'WebinarsController@myBatchEnrollments']);
    Route::get('/webinars/{webinarId}/intake', ['uses' => 'CourseIntakeController@show']);
    Route::post('/webinars/{webinarId}/intake', ['uses' => 'CourseIntakeController@store', 'middleware' => 'api.request.type']);
    Route::post('/webinars/{webinarId}/intake/upload', ['uses' => 'CourseIntakeController@upload']);
    ...
    */
    ...
    Route::group(['prefix' => 'payments'], function () {
        ...
        Route::post('/razorpay-order', 'PaymentsController@createRazorpayOrder');
    ...
    */
    ...
    Route::group(['prefix' => 'webinars'], function () {
        ...
        Route::group(['middleware' => 'api.level-access:teacher'], function () {
            Route::get('/{id}/batches', ['uses' => 'CourseBatchController@index']);
            Route::post('/{id}/batches', ['uses' => 'CourseBatchController@store']);
            Route::put('/{id}/batches/{batch_id}', ['uses' => 'CourseBatchController@update']);
            Route::delete('/{id}/batches/{batch_id}', ['uses' => 'CourseBatchController@destroy']);
        });
    ...
    /***** Fit Karnataka: Daily Health Logs *****/
    Route::group(['prefix' => 'health-logs'], function () {
        Route::get('/', ['uses' => 'StudentDailyHealthLogController@index']);
        Route::post('/', ['uses' => 'StudentDailyHealthLogController@store', 'middleware' => 'api.request.type']);
        Route::get('/summary', ['uses' => 'StudentDailyHealthLogController@summary']);
        Route::get('/{id}', ['uses' => 'StudentDailyHealthLogController@show']);
        Route::put('/{id}', ['uses' => 'StudentDailyHealthLogController@update', 'middleware' => 'api.request.type']);
        Route::delete('/{id}', ['uses' => 'StudentDailyHealthLogController@destroy']);
    });
    Route::get('course-health-log-settings/{webinar_id}', ['uses' => 'CourseHealthLogSettingController@show']);

    /** Dietician: list my students (unique buyers enrolled in my courses) */
    Route::get('students', ['uses' => 'StudentsListController@index', 'middleware' => ['api.level-access:teacher']]);
    /** Dietician: view a student's health profile (onboarding data) for students in dietician's courses */
    Route::get('students/{user_id}/health-profile', ['uses' => 'StudentHealthProfileController@show', 'middleware' => ['api.level-access:teacher']]);

    /** Health Care: dietician view of students by purchased course, mark initial conversation */
    Route::get('health-care', ['uses' => 'HealthCareController@index', 'middleware' => ['api.level-access:teacher']]);
    Route::get('health-care/sales/{saleId}/intake', ['uses' => 'HealthCareController@showIntake', 'middleware' => ['api.level-access:teacher']]);
    Route::post('health-care/sales/{saleId}/initial-conversation', ['uses' => 'HealthCareController@markInitialConversation', 'middleware' => ['api.level-access:teacher']]);
});
