...
/* Root "/" - serve React SPA when SERVE_REACT_FROM_BACKEND=true, else JSON or redirect to FRONTEND_URL. */
Route::get('/', function () {
    try {
    ...
    return response()->json([
        'ok' => true,
        'message' => 'Backend is running. Set SERVE_REACT_FROM_BACKEND=true and copy frontend to public/spa/, or set FRONTEND_URL.',
        'api' => $apiUrl,
    ], 200);
})->middleware([]);
...
/* Emergency Database Update route is now in RouteServiceProvider to bypass all middleware */
...
Route::group(['namespace' => 'Auth', 'middleware' => ['check_mobile_app', 'share', 'check_maintenance']], function () {
    ...
    Route::get('/verify-email', 'VerificationController@verifyMagicLink')
...
Route::group(['namespace' => 'Web', 'middleware' => ['check_mobile_app', 'impersonate', 'share', 'check_maintenance']], function () {
    ...
    Route::group(['middleware' => 'web.auth'], function () {
        ...
        // Laravel File Manager: only registered under admin (routes/admin.php) to avoid duplicate route names when using route:cache
        // If instructors need it, they can use the admin file manager URL or use API uploads.
        ...
        Route::group(['prefix' => 'cart'], function () {
            ...
            /** Used by api/checkout.blade.php (Razorpay flow from frontend): create order and show payment page */
            Route::post('/storeCheckout', 'CartController@storeCheckout')->name('cart.storeCheckout');
        });
