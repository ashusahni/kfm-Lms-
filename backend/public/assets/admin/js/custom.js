...
(function ($) {
    ...
    function getLfmPrefix() {
        var fromBody = document.body && document.body.getAttribute('data-lfm-prefix');
        if (fromBody) return fromBody;
        if (typeof window.LARAVEL_FILEMANAGER_PREFIX !== 'undefined' && window.LARAVEL_FILEMANAGER_PREFIX) {
            return window.LARAVEL_FILEMANAGER_PREFIX;
        }
        try {
            var first = (window.location.pathname || '/').split('/').filter(Boolean)[0];
            if (first) {
                return '/' + first + '/laravel-filemanager';
            }
        } catch (e) {}
    ...
    $('body').on('click', '.admin-file-manager', function (e) {
        ...
        e.stopPropagation();
        var $btn = $(this);
        var explicit = $btn.attr('data-prefix') || $btn.data('prefix');
        var prefix = explicit || getLfmPrefix();
        var inputId = $btn.attr('data-input');
        var previewId = $btn.attr('data-preview');
        var targetInput = inputId ? $('#' + inputId) : null;
        var targetPreview = previewId ? $('#' + previewId) : null;
        window.SetUrl = function (items) {
            if (targetInput && items && items.length) {
                var file_path = items.map(function (item) { return item.url; }).join(',');
                targetInput.val(file_path).trigger('change');
            }
            if (targetPreview && targetPreview.length && items && items.length) {
                targetPreview.empty();
                items.forEach(function (item) {
                    targetPreview.append($('<img>').css('height', '5rem').attr('src', item.thumb_url || item.url));
                });
            }
        };
