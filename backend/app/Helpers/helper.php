...
function getNavbarLinks()
{
    ...
    if (!empty($links)) {
        // Exclude Store and Forums from navbar (features removed)
        $links = array_values(array_filter($links, function ($item) {
            $path = parse_url($item['link'] ?? '', PHP_URL_PATH);
            $path = '/' . trim($path ?? '', '/');
            $isForums = ($path === '/forums' || strpos($path, '/forums/') === 0);
            $isProducts = ($path === '/products' || strpos($path, '/products/') === 0);
            return !$isForums && !$isProducts;
        }));
                ...
                if (!empty($user) and !empty($user->email)) {
                    ...
                    if (!empty($mailHost)) {
                        try {
                            ...
                            // Log or ignore so notification still succeeds (e.g. mail config invalid)
                        }
                    }
                }
            }
...
function sendNotificationToEmail($template, $options, $email)
{
    ...
    if (!empty($notificationTemplate)) {
        ...
        if (env('APP_ENV') == 'production' && !empty(env('MAIL_HOST'))) {
            try {
                \Mail::to($email)->send(new \App\Mail\SendNotifications(['title' => $title, 'message' => $message]));
            } catch (\Throwable $e) {
                // Log or ignore so caller does not fail on mail errors
            }
        }
...
/**
 * Fit Karnataka: check if the platform is in Fit Karnataka (health/diet/coaching) mode.
 */
function isFitKarnatakaEnabled()
{
    return config('fit_karnataka.enabled', false);
}

/**
 * Fit Karnataka: check if a feature is disabled (should be hidden/blocked).
 * @param string $feature One of: forum, store_products, reward_points, gift, affiliate_referral,
 *   instructor_finder, organizations, subscription_plans, ad_banners, bundles, instructor_blog, become_instructor_public
 */
function fitKarnatakaFeatureDisabled($feature)
{
    if (!isFitKarnatakaEnabled()) {
        return false;
    }
    $disable = config('fit_karnataka.disable', []);
    return !empty($disable[$feature]);
}

/**
 * Fit Karnataka: filter home sections to only allowed ones.
 * @param \Illuminate\Support\Collection $sections
 * @return \Illuminate\Support\Collection
 */
function fitKarnatakaFilterHomeSections($sections)
{
    if (!isFitKarnatakaEnabled() || !$sections) {
        return $sections;
    }
    $hide = config('fit_karnataka.home_sections_hide', []);
    return $sections->filter(function ($section) use ($hide) {
        return !in_array($section->name, $hide);
    });
}

/**
 * Fit Karnataka: get terminology override (e.g. "Programs" instead of "Courses").
 * @param string $key e.g. courses, course, assignments, assignment, reviews, review
 * @return string|null
 */
function fitKarnatakaTerm($key)
{
    if (!isFitKarnatakaEnabled()) {
        return null;
    }
    $terms = config('fit_karnataka.terminology', []);
