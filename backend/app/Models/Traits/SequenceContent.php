...
trait SequenceContent
{
    ...
    public function checkSequenceContent($user = null, $test = false)
    {
        ...
        $webinar = null;
        if (!empty($this->webinar_id) && method_exists($this, 'webinar')) {
            $webinar = $this->webinar;
        }
        if (!empty($webinar) && !empty($webinar->sequential_module_unlock)) {
            $chapter = $this->chapter ?? null;
            if (!empty($chapter)) {
                $chapterIndex = $webinar->getChapterOrderIndex($chapter->id);
                if ($chapterIndex !== null) {
                    $unlockedIndex = $webinar->getUnlockedChapterIndexForUser($user);
                    if ($chapterIndex > $unlockedIndex) {
                        $result = $result ?? [];
                        $result['all_passed_items_error'] = trans('update.you_should_pass_the_previous_chapter_to_view_this_chapter_parts');
                        return $result;
                    }
                }
            }
