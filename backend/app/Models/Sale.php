...
class Sale extends Model
{
    ...
    {
        if (!empty($orderItem->webinar_id) && !empty($orderItem->batch_id)) {
            $batch = \App\Models\CourseBatch::where('id', $orderItem->batch_id)
                ->where('webinar_id', $orderItem->webinar_id)
                ->first();
            if (!$batch) {
                throw new \RuntimeException(__('Batch is invalid or no longer available.'));
            }
            if (!$batch->isOpen()) {
                throw new \RuntimeException(__('This batch is no longer open for enrollment.'));
            }
            if ($batch->isFull()) {
                throw new \RuntimeException(__('This batch has reached capacity.'));
            }
        }
        ...
        $sale = Sale::create([
            ...
            'batch_id' => (empty($orderItem->gift_id) and !empty($orderItem->webinar_id) and !empty($orderItem->batch_id)) ? $orderItem->batch_id : null,
