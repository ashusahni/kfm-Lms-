...
class FileController extends Controller
{
    public function store(Request $request)
    {
        ...
        $localFileInput = $request->file('local_file');
        $data = $request->get('ajax')['new'];
        $data['s3_file'] = $s3FileInput;
        $data['local_file'] = $localFileInput;
        ...
        $rules = [
            ...
            'file_path' => Rule::requiredIf(!in_array($data['storage'], ['s3', 'secure_host']) && !$request->hasFile('local_file')),
        ...
        if ($data['storage'] == 'upload' && $request->hasFile('local_file')) {
            $rules['local_file'] = 'required|file|max:' . (getFeaturesSettings('max_upload_file_size') ?? 10240);
        }
        ...
        if (!empty($webinar)) {
            ...
            // When storage is 'upload', allow uploading file from local device
            if ($data['storage'] == 'upload' && $request->hasFile('local_file')) {
                $uploadResult = $this->uploadLocalCourseFile($request->file('local_file'), $user->id);
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!empty($webinar) and !empty($file)) {
            ...
            // When storage is 'upload', allow uploading file from local device
            if ($data['storage'] == 'upload' && $request->hasFile('local_file')) {
                $uploadResult = $this->uploadLocalCourseFile($request->file('local_file'), $file->creator_id);
                if (!$uploadResult['status']) {
                    return $uploadResult['response'];
                }
                $data['file_path'] = $uploadResult['path'];
    ...
    /**
     * Upload a course file from local device to public storage.
     */
    private function uploadLocalCourseFile($file, $userId)
    {
        $result = [
            'status' => true,
            'path' => null
        ...
        try {
            ...
            $directory = 'uploads/course_files/' . $userId;
