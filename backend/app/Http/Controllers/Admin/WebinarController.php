...
class WebinarController extends Controller
{
    ...
    public function create()
    {
        ...
        $dietitians = User::whereIn('role_name', [Role::$teacher, Role::$organization])
            ->select('id', 'full_name')
            ->orderBy('full_name')
            ->get();
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'challenge_type' => 'nullable|in:7_days,30_days',
            'program_domain' => 'nullable|in:general,health,fitness,nutrition,wellness',
        ]);
        ...
        $webinar = Webinar::create([
            ...
            'sequential_module_unlock' => !empty($data['sequential_module_unlock']),
            'price' => $data['price'],
            'organization_price' => $data['organization_price'] ?? null,
            'points' => $data['points'] ?? null,
            'category_id' => $data['category_id'],
            'program_domain' => $data['program_domain'] ?? 'general',
        ...
        $webinar->assignedDieticians()->sync($request->input('dietician_ids', []));

        $programDomain = $data['program_domain'] ?? 'general';
        if (in_array($programDomain, ['health', 'fitness', 'nutrition', 'wellness'], true)) {
            \App\Models\CourseHealthLogSetting::firstOrCreate(
                ['webinar_id' => $webinar->id],
                [
    ...
    public function edit(Request $request, $id)
    {
        ...
        $data = [
            ...
            'dietitians' => User::whereIn('role_name', [Role::$teacher, Role::$organization])
                ...
                ->orderBy('full_name')
                ->get(),
            'assignedDieticianIds' => $webinar->assignedDieticians->pluck('id')->toArray(),
    ...
    public function search(Request $request)
    {
        ...
        $query = \Illuminate\Support\Facades\DB::table('webinars')
            ->join('webinar_translations', function ($join) use ($locale) {
                $join->on('webinars.id', '=', 'webinar_translations.webinar_id')
                    ->where('webinar_translations.locale', '=', $locale);
            })
            ->where('webinar_translations.title', 'like', '%' . $term . '%')
            ->select('webinars.id', 'webinar_translations.title');

        if (!empty($option) and $option == 'just_webinar') {
            $query->where('webinars.type', Webinar::$webinar);
            $query->where('webinars.status', Webinar::$active);
        }
