...
class UserController extends Controller
{
    ...
    public function students(Request $request, $is_export_excel = false)
    {
        ...
        $query = User::where(function ($q) use ($userRoleId) {
            $q->where('role_name', Role::$user)->orWhere('role_id', $userRoleId);
        });
    ...
    public function instructors(Request $request, $is_export_excel = false)
    {
        ...
        $data = [
            'pageTitle' => trans('admin/main.dietitians'),
            'users' => $users,
    ...
    /**
     * Show form to assign courses (webinars) to a dietician. Only for teacher/organization users.
     */
    public function assignCourses($id)
    {
        ...
        try {
            ...
            $webinars = DB::table('webinars')
                ->leftJoin('webinar_translations', function ($join) use ($locale) {
                    $join->on('webinars.id', '=', 'webinar_translations.webinar_id')
                        ->where('webinar_translations.locale', '=', $locale);
                })
                ->where('webinars.status', Webinar::$active)
                ->orderByRaw('COALESCE(webinar_translations.title, CONCAT("Course #", webinars.id)) ASC')
                ->select('webinars.id', DB::raw('COALESCE(webinar_translations.title, CONCAT("Course #", webinars.id)) as title'))
                ->get();
        } catch (\Throwable $e) {
            \Log::warning('assignCourses: join with webinar_translations failed, using fallback', ['error' => $e->getMessage()]);
            $webinars = DB::table('webinars')
                ->where('status', Webinar::$active)
                ->orderBy('id')
                ->select('webinars.id', DB::raw('CONCAT("Course #", webinars.id) as title'))
                ->get();
        }

        $webinars = collect($webinars);
        $assignedIds = $user->assignedWebinars()->pluck('webinars.id')->toArray();

        $data = [
            'pageTitle' => trans('admin/main.assign_courses') . ' - ' . $user->full_name,
            'user' => $user,
            'webinars' => $webinars,
            'assignedIds' => $assignedIds,
        ];

        return view('admin.users.dietician_assign_courses', $data);
    }

    /**
     * Save course assignments for a dietician. Syncs webinar_dietician pivot.
     */
    public function postAssignCourses(Request $request, $id)
    {
        $this->authorize('admin_instructors_list');
        ...
        $webinarIds = $request->input('webinar_ids', []);
        if (!is_array($webinarIds)) {
            $webinarIds = [];
        }
        $webinarIds = array_map('intval', array_filter($webinarIds));

        $user->assignedWebinars()->sync($webinarIds);

        $toastData = [
            'title' => trans('public.request_success'),
            'msg' => trans('admin/main.courses_assigned_success'),
            'status' => 'success',
        ];
        return redirect(getAdminPanelUrl() . '/dieticians/' . $id . '/assign-courses')->with(['toast' => $toastData]);
    }
