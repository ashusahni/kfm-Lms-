...
class VerificationController extends Controller
{
    ...
    /**
     * Verify email via magic link (signed URL). Used when VERIFICATION_METHOD=magic_link.
     */
    public function verifyMagicLink(Request $request)
    {
        ...
        if (!$verification) {
            return redirect(frontend_url('/login?verified=0'));
        }
        ...
        $username = !empty($verification->email) ? 'email' : 'mobile';
        $value = $verification->email ?? $verification->mobile;
        ...
        return redirect(frontend_url('/login?verified=1'));
    }
}
