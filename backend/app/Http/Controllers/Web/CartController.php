...
class CartController extends Controller
{
    ...
    /**
     * Used by api/checkout.blade.php when user comes from frontend (Razorpay flow).
     * Creates order from cart and shows payment page. No address validation.
     */
    public function storeCheckout(Request $request)
    {
        $user = auth()->user();
        $carts = Cart::where('creator_id', $user->id)
            ->with(['productOrder' => function ($q) {
        ...
        if (!empty($order) && $order->total_amount > 0) {
            ...
            foreach ($paymentChannels as $paymentChannel) {
                if ($paymentChannel->class_name == 'Razorpay' && (!$isMultiCurrency || in_array(currency(), $paymentChannel->currencies ?? []))) {
                    $razorpay = true;
                }
            }
            $totalCashbackAmount = $this->getTotalCashbackAmount($carts, $user, $calculate['sub_total']);
