    ...
    public function playFile($slug, $file_id)
    {
        ...
        if (!empty($webinar) and $this->checkCanAccessToPrivateCourse($webinar)) {
            ...
            if (!empty($file)) {
                ...
                if ($canAccess && !$webinar->canUserAccessCourseContent(auth()->user())) {
                    abort(403, 'Course content does not open immediately. Submit the course intake form first; then content unlocks 48 hours after submission so your dietician can review your case and prepare your diet plan.');
                }
