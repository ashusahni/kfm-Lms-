    ...
    public function storeUserWebinarCart($user, $data)
    {
        ...
        if (!empty($webinar) and !empty($user)) {
            ...
            if ($hasOpenBatches && empty($batch_id)) {
                $toastData = ['title' => trans('public.request_failed'), 'msg' => trans('api.cart.batch_required') ?? 'Please select a batch for this course.', 'status' => 'error'];
            ...
            if (!empty($batch_id)) {
                ...
                if (!$batch) {
                    $toastData = ['title' => trans('public.request_failed'), 'msg' => trans('api.cart.invalid_batch') ?? 'Invalid batch for this course.', 'status' => 'error'];
                    return back()->with(['toast' => $toastData]);
                }
                if (!$batch->isOpen()) {
                    $toastData = ['title' => trans('public.request_failed'), 'msg' => trans('api.cart.batch_not_open') ?? 'This batch is not open for enrollment.', 'status' => 'error'];
                    return back()->with(['toast' => $toastData]);
                }
            }
