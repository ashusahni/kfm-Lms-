...
use App\PaymentChannels\Drivers\Razorpay\Channel as RazorpayChannel;
use App\User;
...
class PaymentsController extends Controller
{
    ...
    public function paymentByCredit(Request $request)
    {
        ...
        $orderAmount = $order->total_amount ?? $order->amount ?? 0;
        if ($user->getAccountingCharge() < $orderAmount) {
    ...
    /**
     * Create a Razorpay order for frontend (React) checkout. Returns data to open Razorpay Checkout in the app.
     */
    public function createRazorpayOrder(Request $request)
    {
        validateParam($request->all(), [
            'order_id' => ['required', Rule::exists('orders', 'id')->where('status', Order::$pending)->where('user_id', apiAuth()->id)],
        ]);
        $user = apiAuth();
        $order = Order::where('id', $request->input('order_id'))->where('user_id', $user->id)->with('user')->first();
        if (!$order) {
            return apiResponse2(0, 'invalid_order', trans('api.payment.invalid_order'));
        }
        $channel = PaymentChannel::where('class_name', 'Razorpay')->where('status', 'active')->first();
        if (!$channel) {
            return apiResponse2(0, 'gateway_disabled', trans('api.payment.disabled_gateway'));
        }
        $razorpayChannel = new RazorpayChannel($channel);
        $amount = $razorpayChannel->makeAmountByCurrency($order->total_amount, currency());
        $amountInSubunits = (int) round($amount * 100);
        if ($amountInSubunits < 1) {
            return apiResponse2(0, 'invalid_amount', 'Payment amount must be at least 1 subunit.');
        }
        $apiKey = config('payment.razorpay.api_key') ?: env('RAZORPAY_API_KEY');
        $apiSecret = config('payment.razorpay.api_secret') ?: env('RAZORPAY_API_SECRET');
        if (empty($apiKey) || empty($apiSecret)) {
            return apiResponse2(0, 'razorpay_not_configured', 'Razorpay is not configured. Set RAZORPAY_API_KEY and RAZORPAY_API_SECRET in .env.');
        }
        try {
            ...
            $razorpayOrder = $api->order->create([
                ...
                'notes' => ['lms_order_id' => (string) $order->id],
            ]);
        } catch (\Razorpay\Api\Errors\BadRequestError $e) {
            $msg = $e->getMessage();
            if (stripos($msg, 'Authentication failed') !== false) {
                return apiResponse2(0, 'razorpay_auth_failed', 'Razorpay authentication failed. Check RAZORPAY_API_KEY and RAZORPAY_API_SECRET in backend .env and use valid keys from https://dashboard.razorpay.com/app/keys');
            }
            return apiResponse2(0, 'razorpay_error', $msg ?: 'Razorpay request failed.');
        } catch (\Throwable $e) {
            return apiResponse2(0, 'razorpay_error', 'Could not create Razorpay order. ' . $e->getMessage());
        }
        $verifyBase = url('/api/development/panel/payments/verify/Razorpay');
        $verifyUrl = $verifyBase.'?order_id='.$order->id;
        return apiResponse2(1, 'created', 'Razorpay order created', [
    ...
    public function paymentVerify(Request $request, $gateway)
    {
        ...
        try {
            ...
            if (!empty($order)) {
                ...
                $frontendUrl = rtrim(env('FRONTEND_URL', config('app.frontend_url', '')), '/');
                $successPath = '/panel/cart/success?order_id='.$order->id;
                if (!empty($frontendUrl)) {
                    return redirect($frontendUrl.$successPath);
                }
                return redirect($successPath);
            } else {
                $orderId = !empty($order) ? $order->id : '';
                $frontendUrl = rtrim(env('FRONTEND_URL', config('app.frontend_url', '')), '/');
                $failedPath = '/panel/cart/failed?order_id='.$orderId;
                if (!empty($frontendUrl)) {
                    return redirect($frontendUrl.$failedPath);
                }
