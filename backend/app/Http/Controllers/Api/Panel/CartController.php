...
class CartController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $hasOpenBatches = CourseBatch::where('webinar_id', $webinar_id)->where('status', CourseBatch::STATUS_OPEN)->exists();
        if ($hasOpenBatches && empty($batch_id)) {
            return apiResponse2(0, 'batch_required', trans('api.cart.batch_required') ?? 'Please select a batch for this course.');
        }

        if (!empty($batch_id)) {
            $batch = CourseBatch::where('id', $batch_id)->where('webinar_id', $webinar_id)->first();
            if (!$batch) {
                return apiResponse2(0, 'invalid_batch', trans('api.cart.invalid_batch') ?? 'Invalid batch for this course.');
            }
            if (!$batch->isOpen()) {
                return apiResponse2(0, 'batch_not_open', trans('api.cart.batch_not_open') ?? 'This batch is not open for enrollment.');
            }
        }
    ...
    {
        $params = ['user' => apiAuth()->id];
        if ($request->has('discount_id')) {
            $params['discount_id'] = $request->input('discount_id');
        }
        $checkoutUrl = URL::signedRoute('my_api.web.checkout', $params);
