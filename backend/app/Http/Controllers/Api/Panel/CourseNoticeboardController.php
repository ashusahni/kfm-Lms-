...
class CourseNoticeboardController extends Controller
{
    public function index($webinar_id)
    {
        ...
        $user = apiAuth();
        $canManage = in_array($webinar->id, $user->getManageableWebinarIds());
        // noticeboards: allow creator, teacher, assigned dietician, or admin
        if ($webinar->creator_id != $user->id and $webinar->teacher_id != $user->id and !$canManage and !$user->isAdmin()) {
