...
class WebinarsController extends Controller
{
    ...
    /**
     * Student: list my batch enrollments (sales with batch_id).
     */
    public function myBatchEnrollments()
    {
        $user = apiAuth();
        $sales = \App\Models\Sale::where('buyer_id', $user->id)
            ...
            ->orderBy('created_at', 'desc')
            ->get();
        $enrollments = $sales->map(function ($sale) {
            $webinar = $sale->webinar;
            $batch = $sale->batch;
            return [
                'sale_id' => $sale->id,
                'webinar_id' => $sale->webinar_id,
                'webinar_title' => $webinar ? $webinar->title : null,
                'learning_url' => $webinar ? $webinar->getLearningPageUrl() : null,
                'batch_id' => $sale->batch_id,
                'batch_name' => $batch ? $batch->name : null,
                'batch_code' => $batch ? $batch->code : null,
            ];
        });
        return apiResponse2(1, 'retrieved', trans('api.public.retrieved'), ['enrollments' => $enrollments]);
    }
