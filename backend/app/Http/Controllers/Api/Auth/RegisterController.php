...
class RegisterController extends Controller
{
    ...
    */
    ...
    {
        $allowedMethod = getGeneralSettings('register_method') ?? 'mobile';
        $showOtherMethod = (bool) (getGeneralSettings('show_other_register_method') ?? false);
        $data = $request->all();
        $username = $this->username();

        if (!$username) {
            return apiResponse2(0, 'validation_error', trans('api.auth.send_email_or_mobile'));
        }

        // Allow both email and mobile registration so users can sign up with either
        $acceptBoth = true;
        if (!$acceptBoth && $allowedMethod !== $username) {
            $message = $allowedMethod === 'email'
                ? trans('api.auth.registration_email_only')
                : trans('api.auth.registration_mobile_only');
            return apiResponse2(0, 'invalid_register_method', $message);
        }
        ...
        if ($userCase) {
            $skipVerification = (bool) config('app.registration_skip_verification', true);
            if ($skipVerification) {
                $userCase->update([
                    'password' => Hash::make($data['password']),
                    'status' => User::$active,
                    'full_name' => $userCase->full_name ?: ($username === 'email' ? (strstr($data[$username], '@', true) ?: $data[$username]) : 'User'),
                ]);
                $token = auth('api')->tokenById($userCase->id);
                return apiResponse2(1, 'login', trans('api.auth.login'), [
                    'user_id' => $userCase->id,
                    'token' => $token,
                    'role_name' => $userCase->role_name ?? Role::$user,
                ]);
            }
            ...
            if ($checkConfirmed['status'] == 'verified') {
                ...
                $userCase->update(['password' => Hash::make($data['password'])]);
                return apiResponse2(1, 'go_step_2', trans('api.auth.go_step_2'), [
                    'user_id' => $userCase->id
                ]);
            }
        ...
        if ($skipVerification) {
            $user->update([
                'status' => User::$active,
                'full_name' => $username === 'email' ? strstr($data[$username], '@', true) ?: $data[$username] : 'User',
            ]);
            $token = auth('api')->tokenById($user->id);
            return apiResponse2(1, 'login', trans('api.auth.login'), [
                'user_id' => $user->id,
                'token' => $token,
                'role_name' => $user->role_name ?? Role::$user,
            ]);
        }
        ...
        $status = ($checkConfirmed['status'] === 'verified') ? 'go_step_3' : 'go_step_2';
        return apiResponse2(1, $status, trans('api.public.stored'), array_filter([
            'user_id' => $user->id,
            'debug_code' => $checkConfirmed['debug_code'] ?? null,
        ]));
    }
