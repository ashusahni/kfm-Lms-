...
class WebinarsController extends Controller
{
    ...
    public function storeAll(Request $request)
    {
        ...
        $rules = [
            'type' => 'required|in:webinar,course,text_lesson',
            'title' => 'required|max:255',
            'thumbnail' => 'required_unless:draft,1',
            'image_cover' => 'required_unless:draft,1',
            'description' => 'required',
            'category_id' => 'required|exists:categories,id',
            'duration' => 'required|numeric|min:0|max:8760',
            'start_date' => 'required_if:type,webinar|date',
            'capacity' => 'required_if:type,webinar|numeric|min:0|max:99999',
        ...
        $isDraftRequest = (!empty($data['draft']) && $data['draft'] == 1);
        $slug = Webinar::makeSlug($data['title']);
        $thumbnail = $data['thumbnail'] ?? null;
        $imageCover = $data['image_cover'] ?? null;
        if ($isDraftRequest && !$thumbnail) {
            $thumbnail = '/assets/default/img/cover/webinar_placeholder.jpg';
        }
        if ($isDraftRequest && !$imageCover) {
            $imageCover = '/assets/default/img/cover/webinar_placeholder.jpg';
        }
        $webinar = Webinar::create([
            'teacher_id' => $user->isTeacher() ? $user->id : $data['teacher_id'],
            'creator_id' => $user->id,
            ...
            'video_demo' => $data['video_demo'] ?? null,
            'description' => $data['description'],
            'status' => ($isDraftRequest || (!empty($data['get_next']) and $data['get_next'] == 1)) ? Webinar::$isDraft : Webinar::$pending,
            'created_at' => time(),
        ]);
        ...
        $data['program_domain'] = $data['program_domain'] ?? 'general';
