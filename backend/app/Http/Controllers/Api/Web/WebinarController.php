...
class WebinarController extends Controller
{
    ...
    public function content($id)
    {
        ...
        $canAccessContent = $webinar->canUserAccessCourseContent($user);
        if ($user && $hasBought && !$canAccessContent) {
            $sale = $webinar->getSaleItem($user);
            $initialConversationAt = $sale && isset($sale->initial_conversation_at) ? (int) $sale->initial_conversation_at : null;
            $unlockAfter = $webinar->getContentUnlockAfterTimestamp($user);
            $intake = \App\Models\CourseIntake::where('user_id', $user->id)->where('webinar_id', $webinar->id)->first();

            $lockedReason = 'content_locked_intake_required';
            $message = 'Course content does not open immediately. Please submit the course intake form first. After submission, content will unlock 48 hours later so your dietician can review your case and prepare your diet plan.';

            if ($intake && $unlockAfter !== null) {
                if ($initialConversationAt) {
                    $lockedReason = 'content_locked_conversation';
                    $message = 'Your dietician has not yet marked the initial conversation as complete. Content will unlock after that.';
                } elseif (time() < $unlockAfter) {
                    $lockedReason = 'content_locked_48h';
                    $hoursLeft = (int) ceil(($unlockAfter - time()) / 3600);
                    $message = sprintf('Content unlock delay is 48 hours after course form submission (%d hours remaining). This allows your dietician to review your case and prepare your diet plan. Content will also unlock when your dietician marks the initial conversation complete.', $hoursLeft);
                }
            }

            return apiResponse2(1, 'retrieved', trans('api.public.retrieved'), [
                'webinar' => ['id' => $webinar->id, 'slug' => $webinar->slug, 'title' => $webinar->title],
                'content' => [],
                'content_locked' => true,
                'locked_reason' => $lockedReason,
                'locked_message' => $message,
                'unlock_after_timestamp' => $unlockAfter ?? 0,
                'intake_submitted' => (bool) $intake,
                'initial_conversation_done' => (bool) $initialConversationAt,
            ]);
        }

        if ($user && !$hasBought) {
            return apiResponse2(0, 'not_purchased', trans('api.webinar.not_purchased'), null);
        }

        $sequentialModuleUnlock = !empty($webinar->sequential_module_unlock);
        $unlockedChapterIndex = 0;
        if ($sequentialModuleUnlock && $user) {
            $unlockedChapterIndex = $webinar->getUnlockedChapterIndexForUser($user);
        }

        $lessonUnlockService = app(\App\Services\LessonUnlockService::class);

        $chapters = collect(WebinarChapterResource::collection($webinar->chapters))->map(function ($item, $index) use ($sequentialModuleUnlock, $unlockedChapterIndex, $lessonUnlockService, $webinar, $user) {
            $arr = array_merge(['type' => 'chapter'], $item->toArray(request()));
            if ($sequentialModuleUnlock) {
                $arr['unlocked'] = $index <= $unlockedChapterIndex;
                $arr['chapter_index'] = $index;
            }
            if (!empty($arr['items']) && is_array($arr['items'])) {
                $itemsWithUnlock = [];
                foreach ($arr['items'] as $i) {
                    $contentType = $i['type'] ?? null;
                    $contentId = $i['id'] ?? null;
                    $baseUnlocked = $sequentialModuleUnlock ? ($arr['unlocked'] ?? true) : true;
                    if ($contentType && $contentId) {
                        $status = $lessonUnlockService->getUnlockStatus($user, $webinar->id, $contentType, $contentId);
                        $i['unlocked'] = $baseUnlocked && $status['unlocked'];
                        $i['unlock_message'] = $status['message'];
                        $i['unlock_at'] = $status['unlock_at'];
                        $i['visible'] = $status['visible'];
                        if ($status['visible']) {
                            $itemsWithUnlock[] = $i;
                        }
                    } else {
                        $i['unlocked'] = $baseUnlocked;
                        $i['unlock_message'] = null;
                        $i['unlock_at'] = null;
                        $i['visible'] = true;
                        $itemsWithUnlock[] = $i;
                    }
                }
                $arr['items'] = $itemsWithUnlock;
            }
            return $arr;
        });

        $addUnlockToContent = function ($arr, $contentType) use ($lessonUnlockService, $webinar, $user) {
            $contentId = $arr['id'] ?? null;
            if ($contentId) {
                $status = $lessonUnlockService->getUnlockStatus($user, $webinar->id, $contentType, $contentId);
                $arr['unlocked'] = $status['unlocked'];
                $arr['unlock_message'] = $status['message'];
                $arr['unlock_at'] = $status['unlock_at'];
                $arr['visible'] = $status['visible'];
            } else {
                $arr['unlocked'] = true;
                $arr['unlock_message'] = null;
                $arr['unlock_at'] = null;
                $arr['visible'] = true;
            }
            return $arr;
        ...
        $files = collect(FileResource::collection($webinar->files->whereNull('chapter_id')))->map(function ($item) use ($sequentialModuleUnlock, $addUnlockToContent) {
            $arr = array_merge(['type' => 'file'], $item->toArray(request()));
            if ($sequentialModuleUnlock) {
                $arr['unlocked'] = true;
            }
            $arr = $addUnlockToContent($arr, 'file');
            return $arr['visible'] ? $arr : null;
        })->filter();
        $sessions = collect(SessionResource::collection($webinar->sessions->whereNull('chapter_id')))->map(function ($item) use ($sequentialModuleUnlock, $addUnlockToContent) {
            $arr = array_merge(['type' => 'session'], $item->toArray(request()));
            if ($sequentialModuleUnlock) {
                $arr['unlocked'] = true;
            }
            $arr = $addUnlockToContent($arr, 'session');
            return $arr['visible'] ? $arr : null;
        })->filter();
        $textLessons = collect(TextLessonResource::collection($webinar->textLessons->whereNull('chapter_id')))->map(function ($item) use ($sequentialModuleUnlock, $addUnlockToContent) {
            $arr = array_merge(['type' => 'text_lesson'], $item->toArray(request()));
            if ($sequentialModuleUnlock) {
                $arr['unlocked'] = true;
            }
            $arr = $addUnlockToContent($arr, 'text_lesson');
            return $arr['visible'] ? $arr : null;
        })->filter();
        ...
        if ($sequentialModuleUnlock) {
            $payload['sequential_module_unlock'] = true;
            $payload['unlocked_chapter_index'] = $unlockedChapterIndex;
        }
    ...
    public function learningStatus(Request $request, $webinar_id)
    {
        ...
        if (!empty($course->sequential_module_unlock) && $status) {
            $itemModel = null;
            if ($item === 'file_id') {
                $itemModel = \App\Models\File::where('id', $item_id)->where('webinar_id', $webinar_id)->first();
            } elseif ($item === 'session_id') {
                $itemModel = \App\Models\Session::where('id', $item_id)->where('webinar_id', $webinar_id)->first();
            } else {
                $itemModel = \App\Models\TextLesson::where('id', $item_id)->where('webinar_id', $webinar_id)->first();
            }
            if ($itemModel && !empty($itemModel->chapter_id)) {
                $chapterIndex = $course->getChapterOrderIndex($itemModel->chapter_id);
                if ($chapterIndex !== null) {
                    $unlockedIndex = $course->getUnlockedChapterIndexForUser($user);
                    if ($chapterIndex > $unlockedIndex) {
                        return apiResponse2(0, 'locked', trans('update.you_should_pass_the_previous_chapter_to_view_this_chapter_parts'), null);
                    }
                }
            }
        }
