...
class WebinarPolicy
{
    ...
    {
        // Creator, teacher, assigned dietician, or admin can view (manage) the course
        if (!empty($user) && ($user->id == $webinar->creator_id || $user->organ_id == $webinar->creator_id || $user->id == $webinar->teacher_id || $user->isAdmin())) {
            return true;
        }
        if (!empty($user) && in_array($webinar->id, $user->getManageableWebinarIds())) {
            return true;
        }
        $access = false;
