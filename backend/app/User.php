...
class User extends Authenticatable
{
    ...
    /**
     * Courses assigned to this user (dietitian) by admin. Used for dietician panel access.
     * Select webinars.* explicitly to avoid ambiguous 'id' when joining webinar_dietician.
     */
    public function assignedWebinars()
    {
        return $this->belongsToMany('App\Models\Webinar', 'webinar_dietician', 'user_id', 'webinar_id')
            ->select('webinars.*');
    }

    /**
     * Webinar IDs this user can manage (teacher, creator/org, partner teacher, or assigned dietician).
     */
    public function getManageableWebinarIds()
    {
        $ids = [];
        ...
        if ($this->isTeacher()) {
            $ids = array_merge(
                $ids,
                Webinar::where('teacher_id', $this->id)->pluck('id')->toArray()
            );
        }
        if ($this->isOrganization()) {
            $ids = array_merge(
                $ids,
                Webinar::where('creator_id', $this->id)->pluck('id')->toArray()
            );
        }

        $partnerIds = \App\Models\WebinarPartnerTeacher::where('teacher_id', $this->id)->pluck('webinar_id')->toArray();
        $ids = array_merge($ids, $partnerIds);

        $assignedIds = $this->assignedWebinars()->get()->pluck('id')->toArray();
    ...
    /** Health onboarding relationships */
    public function healthProfile()
    {
        return $this->hasOne(\App\Models\HealthProfile::class);
    }

    public function healthConditions()
    {
        return $this->belongsToMany(\App\Models\HealthCondition::class, 'user_health_conditions', 'user_id', 'condition_id')
            ->withTimestamps();
    }

    public function medicalData()
    {
        return $this->hasOne(\App\Models\MedicalData::class);
    }

    public function dietPattern()
    {
        return $this->hasOne(\App\Models\DietPattern::class);
    }

    public function lifestyleAssessment()
    {
        return $this->hasOne(\App\Models\LifestyleAssessment::class);
    }

    public function bodyGoals()
    {
        return $this->belongsToMany(\App\Models\BodyGoal::class, 'user_body_goals', 'user_id', 'goal_id')
            ->withTimestamps();
    }

    public function files()
    {
        return $this->hasOne(\App\Models\FileUpload::class, 'user_id');
    }
}
