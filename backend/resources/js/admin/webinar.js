(function ($) {
    ...
    function handleFileFormSubmit(form, $this) {
        ...
        const localUploadInput = form.find('.js-local-upload-file');
        ...
        const source = form.find('.js-file-storage').val();
        if (source === 'upload' && localUploadInput && localUploadInput.prop('files') && localUploadInput.prop('files')[0]) {
            formData.append('local_file', localUploadInput.prop('files')[0]);
            hasFileForUpload = true;
        }
        ...
        $.ajax({
            ...
            xhr: function () {
                ...
                xhr.upload.addEventListener("progress", function (event) {
                    if (event.lengthComputable && (source === "s3" || source === "secure_host" || source === "upload") && hasFileForUpload) {
    ...
    function handleShowFileInputsBySource($form, source, fileType) {
        ...
        const $localUploadInputGroup = $form.find('.js-local-upload-file-input');
        ...
        $localUploadInputGroup.addClass('d-none');
        ...
        switch (source) {
            ...
            case 'upload':
                $fileTypeVolumeInputs.removeClass('d-none');
                $localUploadInputGroup.removeClass('d-none');
                if (fileType && (fileType === 'pdf')) {
                    $onlineViewerInput.removeClass('d-none');
                } else {
                    $onlineViewerInput.find('input').prop('checked', false);
