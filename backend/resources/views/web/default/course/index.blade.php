                ...
                <div class="rounded-lg shadow-sm">
                    ...
                    <div class="px-20 pb-30">
                        <form action="/cart/store" method="post">
                            ...
                            @if($course->batches->where('status', 'open')->count() > 0)
                                <div class="form-group mt-20">
                                    <label class="font-16 font-weight-500 text-dark-blue">{{ trans('admin/main.course_batches') ?? 'Select batch' }}</label>
                                    <select name="batch_id" class="form-control">
                                        <option value="">{{ trans('admin/main.select_batch') ?? '— Select batch —' }}</option>
                                        @foreach($course->batches->where('status', 'open') as $batch)
                                            @if($batch->isOpen())
                                                <option value="{{ $batch->id }}">{{ $batch->name }} @if($batch->capacity)({{ $batch->enrolled_count }}/{{ $batch->capacity }})@endif</option>
                                            @endif
                                        @endforeach
                                    </select>
                                </div>
