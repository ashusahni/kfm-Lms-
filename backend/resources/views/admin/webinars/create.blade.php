                                ...
                                <section>
                                    ...
                                    <div class="row">
                                        <div class="col-12 col-md-5">
                                            ...
                                            <div class="form-group mt-15 ">
                                                <label class="input-label d-block">{{ trans('admin/main.challenge_course') ?? 'Challenge course' }}</label>
                                                <select name="challenge_type" class="custom-select @error('challenge_type') is-invalid @enderror">
                                                    <option value="" @if(empty($webinar->challenge_type) && old('challenge_type') !== '7_days' && old('challenge_type') !== '30_days') selected @endif>{{ trans('admin/main.no_challenge') ?? 'No (regular course)' }}</option>
                                                    <option value="7_days" @if((!empty($webinar) && $webinar->challenge_type === '7_days') || old('challenge_type') === '7_days') selected @endif>7 Day Challenge</option>
                                                    <option value="30_days" @if((!empty($webinar) && $webinar->challenge_type === '30_days') || old('challenge_type') === '30_days') selected @endif>30 Day Challenge</option>
                                                </select>
                                                <small class="text-muted">{{ trans('admin/main.challenge_course_hint') ?? 'Course is released in batches by start date. When a customer purchases, they join the selected batch (e.g. Batch starting 1 Mar). Add batches under Course batches.' }}</small>
                                            ...
                                            <div class="form-group mt-15 ">
                                                ...
                                                <select name="program_domain" class="custom-select @error('program_domain') is-invalid @enderror">
                                                    <option value="general" @if((!empty($webinar) && ($webinar->program_domain ?? 'general') === 'general') || old('program_domain', 'general') === 'general') selected @endif>General</option>
                                                    <option value="health" @if((!empty($webinar) && ($webinar->program_domain ?? '') === 'health') || old('program_domain') === 'health') selected @endif>Health</option>
                                                    <option value="fitness" @if((!empty($webinar) && ($webinar->program_domain ?? '') === 'fitness') || old('program_domain') === 'fitness') selected @endif>Fitness</option>
                                                    <option value="nutrition" @if((!empty($webinar) && ($webinar->program_domain ?? '') === 'nutrition') || old('program_domain') === 'nutrition') selected @endif>Nutrition</option>
                                                    <option value="wellness" @if((!empty($webinar) && ($webinar->program_domain ?? '') === 'wellness') || old('program_domain') === 'wellness') selected @endif>Wellness</option>
                                                </select>
                                            ...
                                            @if(!empty($webinar) and $webinar->creator and $webinar->creator->isOrganization())
                                                <div class="form-group mt-15 ">
                                            ...
                                            <div class="form-group mt-15">
                                                ...
                                                <div class="input-group">
                                                    <div class="input-group-prepend">
                                                        <button type="button" class="input-group-text admin-file-manager" data-input="thumbnail" data-preview="holder" data-prefix="{{ getAdminPanelUrl('/laravel-filemanager') }}">
                                            ...
                                            <div class="form-group mt-15">
                                                ...
                                                <div class="input-group">
                                                    <div class="input-group-prepend">
                                                        <button type="button" class="input-group-text admin-file-manager" data-input="cover_image" data-preview="holder" data-prefix="{{ getAdminPanelUrl('/laravel-filemanager') }}">
                                                            <i class="fa fa-upload"></i>
                                                        </button>
                                                    </div>
                                            ...
                                            <div class="form-group mt-0">
                                                ...
                                                <div class="input-group js-video-demo-path-input">
                                                    <div class="input-group-prepend">
                                                        <button type="button" class="js-video-demo-path-upload input-group-text admin-file-manager {{ (empty($webinar) or empty($webinar->video_demo_source) or $webinar->video_demo_source == 'upload') ? '' : 'd-none' }}" data-input="demo_video" data-preview="holder" data-prefix="{{ getAdminPanelUrl('/laravel-filemanager') }}">
                                                            <i class="fa fa-upload"></i>
                                                        </button>
                                        ...
                                        <div class="col-12 col-md-6">
                                            ...
                                            <div class="form-group mt-15">
                                                <div class="custom-control custom-checkbox">
                                                    <input type="checkbox" name="sequential_module_unlock" value="1" class="custom-control-input" id="sequential_module_unlock" @if(!empty($webinar) && $webinar->sequential_module_unlock) checked @endif @if(old('sequential_module_unlock')) checked @endif>
                                                    <label class="custom-control-label" for="sequential_module_unlock">{{ trans('update.sequential_module_unlock') ?? 'Unlock content week by week' }}</label>
                                                </div>
                                                <p class="text-muted small mt-1">{{ trans('update.sequential_module_unlock_hint') ?? 'When enabled, students can only access the first chapter (week/module). After completing all items in it, the next chapter unlocks.' }}</p>
                                            </div>
                                            ...
                                            <div class="form-group mt-30">
                                                <label class="input-label d-block">{{ trans('admin/main.assigned_dietitians') ?? 'Assigned dietitians' }}</label>
                                                <select name="dietician_ids[]" multiple class="form-control select2" data-placeholder="{{ trans('admin/main.select_dietitians_to_manage_course') ?? 'Select dietitians who can manage this course' }}">
                                                    @if(!empty($dietitians))
                                                        @foreach($dietitians as $d)
                                                            <option value="{{ $d->id }}" {{ in_array($d->id, $assignedDieticianIds ?? []) ? 'selected' : '' }}>{{ $d->full_name }}</option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                                <div class="text-muted text-small mt-1">{{ trans('admin/main.assigned_dietitians_hint') ?? 'Assigned dietitians can manage batches and view client health logs for this course.' }}</div>
                                            </div>
