    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item"><a href="{{ getAdminPanelUrl() }}/student-health-logs">{{ trans('admin/main.student_health_logs') ?? 'Student health logs' }}</a></div>
        ...
        <div class="section-body">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <p><strong>{{ trans('admin/main.student') ?? 'Student' }}:</strong> {{ $log->user->full_name ?? '—' }} (ID: {{ $log->user_id }})</p>
                            <p><strong>{{ trans('admin/main.email') ?? 'Email' }}:</strong> {{ $log->user->email ?? '—' }}</p>
                            <p><strong>{{ trans('admin/main.course') ?? 'Course' }}:</strong> {{ $log->webinar->title ?? '—' }} @if($log->webinar_id)(ID: {{ $log->webinar_id }})@endif</p>
                            <p><strong>{{ trans('admin/main.log_date') ?? 'Log date' }}:</strong> {{ $log->log_date ? $log->log_date->format('Y-m-d') : '—' }}</p>
                        </div>
                        <div class="col-md-6">
                ...
                <div class="card-footer">
                    <a href="{{ getAdminPanelUrl() }}/student-health-logs?{{ http_build_query(request()->query()) }}" class="btn btn-secondary">{{ trans('public.back') ?? 'Back' }}</a>
                </div>
            </div>
        </div>
    </section>
