...
<head>
    ...
    <link rel="stylesheet" href="/assets/admin/css/admin-theme.css">
    <?php if($isRtl): ?>
...
<body class="<?php if($isRtl): ?> rtl <?php endif; ?>" data-lfm-prefix="<?php echo e(getAdminPanelUrl('/laravel-filemanager')); ?>">
...
<script>window.LARAVEL_FILEMANAGER_PREFIX = <?php echo e(json_encode(getAdminPanelUrl('/laravel-filemanager'))); ?>;</script>
<!-- Template JS File -->
...
<script>
(function() {
    var prefix = <?php echo e(json_encode(getAdminPanelUrl('/laravel-filemanager'))); ?>;
    function handleLfmClick(e) {
        var btn = e.target.closest('.admin-file-manager');
        if (!btn) return;
        e.preventDefault();
        e.stopPropagation();
        ...
        window.SetUrl = function(items) {
            if (inputId && items && items.length) {
                var el = document.getElementById(inputId);
                if (el) el.value = items.map(function(i) { return i.url; }).join(',');
            }
            if (previewId && items && items.length) {
    ...
    document.body.addEventListener('click', handleLfmClick, true);
})();
</script>
...
<script>
    (function () {
        ...
        if ($('[data-toggle="tooltip"]').length) {
            try { $('[data-toggle="tooltip"]').tooltip({ boundary: 'window' }); } catch (e) {}
        }
    })();
</script>
<script>
...
<?php /**PATH C:\Users\ashut\Downloads\Telegram Desktop\rocket-lms_v1.8 (2)\backend\resources\views/admin/layouts/app.blade.php ENDPATH**/ ?>