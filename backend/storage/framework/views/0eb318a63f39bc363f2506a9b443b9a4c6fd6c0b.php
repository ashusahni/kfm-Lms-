<div class="tab-pane fade <?php if(request()->get('tab') == "payment_channels"): ?> active show <?php endif; ?>" id="payment_channels" role="tabpanel" aria-labelledby="payment_channels-tab">
    <div class="card">
        ...
        <div class="card-body">
            <?php if($paymentChannels->isEmpty()): ?>
                <div class="alert alert-info">
                    <strong>Razorpay</strong> will appear here after you run one of the following in your project root:
                    <ul class="mb-0 mt-2">
                        <li><code>php artisan migrate</code> — runs the migration that adds Razorpay as the only payment channel</li>
                        <li>or <code>php artisan db:seed --class=PaymentChannelsTableSeeder</code> — seeds Razorpay only</li>
                    </ul>
                    <p class="mb-0 mt-2">Set <code>RAZORPAY_API_KEY</code> and <code>RAZORPAY_API_SECRET</code> in <code>.env</code>. See <code>backend/RAZORPAY_INTEGRATION.md</code> for details.</p>
                </div>
            <?php else: ?>
        ...
        <?php if($paymentChannels->isNotEmpty()): ?>
...
<?php /**PATH C:\Users\ashut\Downloads\Telegram Desktop\rocket-lms_v1.8\backend\resources\views/admin/settings/financial/payment_channel/lists.blade.php ENDPATH**/ ?>